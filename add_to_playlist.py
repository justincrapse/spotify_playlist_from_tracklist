...
# token = util.prompt_for_user_token(username=username, scope=scope, client_id=cid, client_secret=secret, redirect_uri="http://localhost:8080")
token = util.prompt_for_user_token(username=username, scope=scope, client_id=cid, client_secret=secret, redirect_uri="http://localhost:3000")
...
with open(f"{current_path}/track_lists/{conf.TRACKLIST}", "r", encoding="utf-8") as file:
    ...
    track_dict_list = []
    for row in reader:
        track_dict_list.append({"artist": row[0].split("|"), "track": row[1], "remixers": row[2].split("|")})
