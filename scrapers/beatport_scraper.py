...
def get_top_100_playlist(genre_url_snippet) -> List[dict]:
    ...
    # parse remixer[s]
    remixer_elems = tree.xpath('//li[@class="bucket-item ec-item track"]//p[@class="buk-track-remixers"]')[:top_count]
    remixer_list = [[re_sp.sub(' ', re_clean.sub('', i.text)) for i in entry.xpath('.//a')] for entry in remixer_elems]
    
    # parse track titles
    track_elements = tree.xpath('//li[@class="bucket-item ec-item track"]//span[@class="buk-track-primary-title"]')[:top_count]
